...
# ----------------------------------------------------------------
    ...
    if st.session_state.architecture_result:
        ...
        mapping = {
            ...
            "Plugin System/Microkernel": "microkernel.png",
            "Hexagonal Architecture": "hexagonal.png",
            "Layered": "layered.png",
            "CQRS": "cqrs.png",
            "Modular Monolith": "modular_monolith.png",
        ...
        matches = [
            ...
            if name.lower() in arch_lower
        ]

        if matches:
            for name, fn in matches:
                img_path = Path("static/diagrams") / fn
                if img_path.exists():
                    st.image(
                        str(img_path),
                        caption=f"{name} architecture diagram generated by AI",
                        width=500
                    )
                else:
                    st.warning(f"Diagram pre “{name}” ({fn}) sa nenašiel.")
        else:
            st.info("Pre danú architektúru nebol nájdený žiadny diagram.")
